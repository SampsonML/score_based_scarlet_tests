...
# Editted by: Matt Sampson initial port by by Patrick Kidger                                                         #
# Created: 2023                                                                  #
# ------------------------------------------------------------------------------ #
...
def HST_data(im_size):
    if im_size == 31: # Matt: change this back to 32 after ZTF trials
        box_size = 31
    ...
    elif im_size == 61: # Matt: change this back to 32 after ZTF trials
        box_size = 51
        dataname = 'sources_box' + str(box_size) + '.npy'     
        dataset = np.load(dataname)
        # perform zero-padding of the data to get desired dimensions
    ...
    # add ZTF data for Sufia/Charlotte project
    elif im_size == 32:
        dataname = 'ztf_32.npy'     
        dataset = np.load(dataname)
        for i in range(len(dataset)):
            dataset[i] = norm_array(dataset[i])
...
def main(
    ...
    # --------------------------- #
    # edit values of training set #
    # --------------------------- #
    #tol = 1e-14
    #data = data + tol
    #data = jnp.log(data)
    
    def normalize_jnp(x, newRange=(0, 1)): #x is an array. Default range is between zero and one
        xmin, xmax = jnp.min(x), jnp.max(x) #get max and min from input array
    ...
    data = data / data_std      # Reduce variance
    # normalising above 0 to eliminate negative pixel issues
    data = normalize_jnp(data, newRange(0.25, 1))   
    
    # print new range
    print(f"minimum pixel value is {jnp.min(data)}")
    print(f"maximum pixel value is {jnp.max(data)}")
    ...
    for step, data in zip(
        ...
        if (step % print_every) == 0 or step == num_steps - 1:
            ...
            total_size = 0
            # save the model
            fn = SAVE_DIR + '/ztf_model_step_' +str(step) + '_res_' +str(args.size)+ '.eqx'
    ...
    #sample = data_std * sample
    print(f'min value in samples is: {jnp.min(sample)}')
    print(f'max value in samples is: {jnp.max(sample)}')
    ...
    plt.axis("off")
    plt.tight_layout()
    filename = 'galaxy_ztf_res' + str(args.size) + '.png'
    plt.savefig(filename,facecolor='black', transparent=False ,dpi = 250)
    filename = 'galaxy_ztf_res' + str(args.size) + '.pdf'
    plt.savefig(filename,facecolor='black', transparent=False ,dpi = 250)   
    plt.show()
    
    sample_exp = jnp.asarray(sample)
    # edit this add real space fits
    fig = plt.figure(figsize=(16, 16), dpi = 250)
    plt.style.use('dark_background')
    title = 'score based generation of equinox models'
    plt.suptitle(title, fontsize = 30)
    plt.imshow(jnp.exp(sample_exp), cmap=cmap)
    plt.axis("off")
    plt.tight_layout()
    filename = 'galaxy_ztf_res' + str(args.size) + '.png'
    plt.savefig(filename,facecolor='black', transparent=False ,dpi = 250)
    filename = 'galaxy_ztf_exp_res' + str(args.size) + '.pdf'
    plt.savefig(filename,facecolor='black', transparent=False ,dpi = 250)   
    plt.show()
